...
copy_all_spond_files() {
	...
	mkdir -p etc/bin
	cp ${CUR_DIR}/../../spilib/src/miner_gate/miner_gate_sp20 etc/bin
	cp ${CUR_DIR}/../../spilib/src/miner_gate/miner_gate_sp30 etc/bin
...
cron()
{
	...
	# run once a day, with arbitrary delay to prevent hourly DDOS on firmware server
	echo '0 0 * * * sleep $(printf "%d\n" 0x`hostname | md5sum | cut -c 1-4`) && /etc/cron.d/pandp_register.sh' >> etc/cron.d/crontabs/root
	echo '1 0 * * * sleep $(printf "%d\n" 0x`hostname | md5sum | cut -c 1-4`) && curl -s --fail  "http://firmware.spondoolies-tech.com/release/latest?id=`cat /board_ver`" > /tmp/fw_update ' >> etc/cron.d/crontabs/root
}
